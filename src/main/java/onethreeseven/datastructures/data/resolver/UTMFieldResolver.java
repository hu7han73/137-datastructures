...
public class UTMFieldResolver extends AbstractStringArrayToFieldResolver<double[]> {
    ...
    private final ProjectionMercator mercatorProjection;

    public UTMFieldResolver(ProjectionUTM utmProjection, int eastingIdx, int northingIdx) {
        super(new int[]{eastingIdx, northingIdx});
        this.utmProjection = utmProjection;
        this.mercatorProjection = new ProjectionMercator();
    }
    ...
    public double[] resolve(String[] in) {
        ...
        return mercatorProjection.geographicToCartesian(latlon[0], latlon[1]);
    }
}
